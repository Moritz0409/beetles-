...
setwd("C:/Users/Ferdinand/Documents/Master/Semester 3/Integration Module/Beetle Data")
...
coleo2018 <- read.csv2("LAK_2018_Coleoptera_B.csv")

coleo2018$date <- as.Date(with(coleo2018, paste(coleo2018$End.Tag, coleo2018$End.Monat, coleo2018$Jahr, sep="-")), "%d-%m-%Y")

coleo2018$Anzahl<- as.numeric(coleo2018$Anzahl)
...
beetles_2018<-coleo2018[ which(coleo2018$Art!='NA'
                               & coleo2018$Art!='spec.'& coleo2018$Art!='???? femoralis'), ]
beetles_2018<-beetles_2018 %>% filter(!(grepl("spec.", Art)))
beetles_2018<-beetles_2018 %>% filter(!(grepl("cf.", Art)))

beetles_2018<-beetles_2018 %>% unite("Artname", Gattung:Art, sep=" ")

beetles_2018$Falle<-as.numeric(beetles_2018$Falle)
...
undet_taxa_2018<-coleo2018 %>% filter((grepl(c("spec.|cf.|NA"), Art)))

undet_taxa_2018$Anzahl<-as.numeric(undet_taxa_2018$Anzahl)

#traps and tree species

traps_treespecies <- read.csv2("crane.env.2017.csv")

df_joined <- merge(coleo2017,
                   traps_treespecies,
                   by.x = "Falle",
                   by.y = "Fallennummer")

df_joined2 <- merge(coleo2018,
                    traps_treespecies,
                    by.x = "Falle",
                    by.y = "Fallennummer")
...
beetle_species <- unique(coleo2017$Species)

species_summary <- coleo2017%>%
  group_by(Species) %>%
  summarise(total_count = sum(Anzahl))

df_joined %>%
  group_by(TreeSp, Stratum) %>%
  summarise(n_beetle_species = n_distinct(Species))

df_joined2 %>%
  group_by(TreeSp, Stratum) %>%
  summarise(n_beetle_species = n_distinct(Art))

#ggplot(coleo2017, aes(Falle, Anzahl))+geom_point()

install.packages("openxlsx")
library(openxlsx)

beetles_traits<-read.xlsx("beetle status.xlsx")

colnames(beetles_traits) <- c("Colkat", "Familie", "Genus", "Art", "Ref", "xylobiont", "Rote.Liste", "Urwaldrelikt")
beetles_traits <- beetles_traits[-1,]

df_joined3 <- merge(df_joined,
                    beetles_traits,     
                    by.x = "FHL",
                    by.y = "Colkat")

df_joined4 <- merge(df_joined2,
                    beetles_traits,     
                    by.x = "FHL",
                    by.y = "Colkat")

xylobionts2017 <- df_joined3 %>%
  filter(TreeSp %in% c("Acer", "Quercus rubra", "Ulmus")) %>%  
  filter(xylobiont == "j") %>%                                       
  group_by(TreeSp) %>%
  summarise(n_xylobiont_species = n_distinct(Species))

xylobionts2018 <- df_joined4 %>%
  filter(TreeSp %in% c("Acer", "Quercus rubra", "Ulmus")) %>%  
  filter(xylobiont == "j") %>%                                       
  group_by(TreeSp) %>%
  summarise(n_xylobiont_species = n_distinct(Art.x))

red_list2017 <- df_joined3 %>%
  filter(TreeSp %in% c("Acer", "Quercus rubra", "Ulmus")) %>%
  group_by(TreeSp, Rote.Liste) %>%
  summarise(n_species = n_distinct(Species), .groups = "drop") %>%
  pivot_wider(
    names_from = Rote.Liste,
    values_from = n_species,
    values_fill = 0       
  )

red_list2017_Quercus_Tilia_Fraxinus <- df_joined3 %>%
  filter(TreeSp %in% c("Quercus", "Tilia", "Fraxinus")) %>%
  group_by(TreeSp, Rote.Liste) %>%
  summarise(n_species = n_distinct(Species), .groups = "drop") %>%
  pivot_wider(
    names_from = Rote.Liste,
...
df_joined4$Rote.Liste[df_joined$Rote.Liste == ""] <- NA
df_joined4 <- df_joined4[-(1:692), ]

red_list2018 <- df_joined4 %>%
  filter(TreeSp %in% c("Acer", "Quercus rubra", "Ulmus")) %>%
  group_by(TreeSp, Rote.Liste) %>%
  summarise(n_species = n_distinct(Art.x), .groups = "drop") %>%
  pivot_wider(
    names_from = Rote.Liste,
    values_from = n_species,
    values_fill = 0       
...
Urwaldrelikte2017 <- df_joined3 %>%
  filter(TreeSp %in% c("Acer", "Quercus rubra", "Ulmus")) %>%
  group_by(TreeSp, Urwaldrelikt) %>%
  summarise(n_species = n_distinct(Species), .groups = "drop") %>%
  pivot_wider(
    names_from = Urwaldrelikt,
    values_from = n_species,
    values_fill = 0       
  )

Urwaldrelikte2017_all_trees <- df_joined3 %>%
  group_by(TreeSp, Urwaldrelikt) %>%
  summarise(n_species = n_distinct(Species), .groups = "drop") %>%
  pivot_wider(
    names_from = Urwaldrelikt,
...
#distinct beetle species 2017
install.packages("reshape2")   
library(reshape2)

beetles_2017_treesp <- df_joined3[df_joined3$TreeSp != "Ground", ] %>%
  select(TreeSp, FHL, Anzahl) %>%
  group_by(TreeSp, FHL) %>%
  summarise(n_total = sum(Anzahl))

beetles_2017_distinct <- beetles_2017_treesp %>%
  select(TreeSp, FHL) %>%   
  group_by(FHL) %>%
  mutate(n_appearance = n()) 

beetles_2017_b <- beetles_2017_treesp %>%
  left_join(beetles_2017_distinct, by = c("TreeSp", "FHL")) %>%
...
#distinct species 2018 -> already done by Long in this fashion

#bootstrapping attempt for distinct species

beetles_2017_treesp_bs <- df_joined3 %>%
  filter(TreeSp != "Ground") %>%
  select(Falle, TreeSp, FHL) %>%
  distinct()
...
trap_combinations2017 <- read_excel("trap_combinations2017.xlsx")

process_trap_combination <- function(traps, data) {
  
  data %>%
    filter(Falle %in% traps) %>%
    select(TreeSp, FHL) %>%
    distinct() %>%
    group_by(FHL) %>%
    mutate(n_appearance = n_distinct(TreeSp)) %>%
    ungroup() %>%
    filter(n_appearance == 1) %>%
    group_by(TreeSp, n_appearance) %>%
    summarise(
      n_beetlesp = n_distinct(FHL),
      .groups = "drop"
    ) %>%
    dcast(
      n_appearance ~ TreeSp,
      value.var = "n_beetlesp",
      fill = 0
    )
}

results_list <- lapply(
  seq_len(nrow(trap_combinations2017)),
  function(i) {
    traps <- unlist(trap_combinations2017[i, -1])
    process_trap_combination(traps, beetles_2017_treesp_bs)
  }
)


names(results_list) <- paste0("bs", trap_combinations2017$bs)
...
beetles_2017_treesp_bs1 <- beetles_2017_treesp_bs %>%
  filter(Falle %in% c(1, 2, 9, 10, 17, 18, 21, 22, 27, 28, 29, 30))

beetles_2017_treesp_bs1 <- beetles_2017_treesp_bs1[, -1]

beetles_appearance_by_tree_bs1 <- beetles_2017_treesp_bs1 %>%
  select(TreeSp, FHL) %>%
  distinct() %>%                     
  group_by(FHL) %>%
  mutate(n_appearance = n_distinct(TreeSp)) %>%
...
beetles_appearance_by_tree_cast_bs1 <-
  dcast(
    beetles_appearance_by_tree_bs1,
    n_appearance ~ TreeSp,
    value.var = "n_beetlesp",
    fill = 0
  )
...
beetles_2017_treesp_bs2 <- beetles_2017_treesp_bs %>%
  filter(Falle %in% c(1, 2, 9, 10, 19, 20, 21, 22, 27, 28, 29, 30))

beetles_2017_treesp_bs2 <- beetles_2017_treesp_bs2[, -1]

beetles_appearance_by_tree_bs2 <- beetles_2017_treesp_bs2 %>%
  select(TreeSp, FHL) %>%
  distinct() %>%                     
  group_by(FHL) %>%
  mutate(n_appearance = n_distinct(TreeSp)) %>%
...
beetles_appearance_by_tree_cast_bs2 <-
  dcast(
    beetles_appearance_by_tree_bs2,
    n_appearance ~ TreeSp,
    value.var = "n_beetlesp",
    fill = 0
  )
...
beetles_2017_treesp_bs3 <- beetles_2017_treesp_bs %>%
  filter(Falle %in% c(1, 2, 9, 10, 23, 24, 21, 22, 27, 28, 29, 30))

beetles_2017_treesp_bs3 <- beetles_2017_treesp_bs3[, -1]

beetles_appearance_by_tree_bs3 <- beetles_2017_treesp_bs3 %>%
  select(TreeSp, FHL) %>%   distinct() %>%  group_by(FHL) %>%  mutate(n_appearance = n_distinct(TreeSp)) %>%
  ungroup() %>% filter(n_appearance == 1) %>%  group_by(TreeSp, n_appearance) %>% summarise(
    n_beetlesp = n_distinct(FHL),    .groups = "drop")

beetles_appearance_by_tree_cast_bs3 <-
  dcast(beetles_appearance_by_tree_bs3, n_appearance ~ TreeSp, value.var = "n_beetlesp",fill = 0  )

#4

beetles_2017_treesp_bs4 <- beetles_2017_treesp_bs %>%
  filter(Falle %in% c(1, 2, 9, 10, 25, 26, 21, 22, 27, 28, 29, 30))

beetles_2017_treesp_bs4 <- beetles_2017_treesp_bs4[, -1]

beetles_appearance_by_tree_bs4 <- beetles_2017_treesp_bs4 %>%
  select(TreeSp, FHL) %>%   distinct() %>%  group_by(FHL) %>%  mutate(n_appearance = n_distinct(TreeSp)) %>%
  ungroup() %>% filter(n_appearance == 1) %>%  group_by(TreeSp, n_appearance) %>% summarise(
    n_beetlesp = n_distinct(FHL),    .groups = "drop")

beetles_appearance_by_tree_cast_bs4 <-
  dcast(beetles_appearance_by_tree_bs4, n_appearance ~ TreeSp, value.var = "n_beetlesp",fill = 0  )

#5

beetles_2017_treesp_bs5 <- beetles_2017_treesp_bs %>%
  filter(Falle %in% c(1, 2, 9, 10, 33, 34, 21, 22, 27, 28, 29, 30))

beetles_2017_treesp_bs5 <- beetles_2017_treesp_bs5[, -1]

beetles_appearance_by_tree_bs5 <- beetles_2017_treesp_bs5 %>%
  select(TreeSp, FHL) %>%   distinct() %>%  group_by(FHL) %>%  mutate(n_appearance = n_distinct(TreeSp)) %>%
  ungroup() %>% filter(n_appearance == 1) %>%  group_by(TreeSp, n_appearance) %>% summarise(
    n_beetlesp = n_distinct(FHL),    .groups = "drop")

beetles_appearance_by_tree_cast_bs5 <-
  dcast(beetles_appearance_by_tree_bs5, n_appearance ~ TreeSp, value.var = "n_beetlesp",fill = 0  )

#6

beetles_2017_treesp_bs6 <- beetles_2017_treesp_bs %>%
  filter(Falle %in% c(1, 2, 11, 12, 17, 18, 21, 22, 27, 28, 29, 30))

beetles_2017_treesp_bs6 <- beetles_2017_treesp_bs6[, -1]

beetles_appearance_by_tree_bs6 <- beetles_2017_treesp_bs6 %>%
  select(TreeSp, FHL) %>%   distinct() %>%  group_by(FHL) %>%  mutate(n_appearance = n_distinct(TreeSp)) %>%
  ungroup() %>% filter(n_appearance == 1) %>%  group_by(TreeSp, n_appearance) %>% summarise(
    n_beetlesp = n_distinct(FHL),    .groups = "drop")

beetles_appearance_by_tree_cast_bs6 <-
  dcast(beetles_appearance_by_tree_bs6, n_appearance ~ TreeSp, value.var = "n_beetlesp",fill = 0  )


#merging the first rows of the beetles_appearance_by_tree_cast sheets

combined_appearance_ <- do.call(
  rbind,
  Map(
    function(x, i) cbind(combination_id = i, x),
    mget(paste0("beetles_appearance_by_tree_cast_bs", 1:6)),
    1:6
  )
)
...
#working out comparison with other species

trap17<- df_joined3 %>%
  group_by(Falle == "17") %>%
  summarise(n_beetle_species = n_distinct(Species))

#QuercusTraps <- c(17,18,19,20,23,24,25,26,33,34)

#QuercusBeetles <- df_joined3 %>%
 # filter(Falle %in% QuercusTraps) %>%
  #group_by(Falle) %>%
...
lower_Quercus_traps_2017 <- df_joined3 %>%
  filter(TreeSp == "Quercus", Stratum == "Lower")%>%
  pull(Falle) %>%
  unique()

upper_Quercus_traps_2017 <- df_joined3 %>%
  filter(TreeSp == "Quercus", Stratum == "Upper")%>%
  pull(Falle) %>%
  unique()

set.seed(123)   
n_pairs <- 100
QuercusPairs2017 <- tibble(
  lower = sample(lower_Quercus_traps_2017, n_pairs, replace = TRUE),
  upper = sample(upper_Quercus_traps_2017, n_pairs, replace = TRUE)
)
...
Quercus_2017pairs_summary <- QuercusPairs2017 %>%
  mutate(
    speciesdiversity = map2_int(lower, upper, ~ {
      df_joined3 %>%
        filter(Falle %in% c(.x, .y)) %>%
        summarise(speciesdiversity = n_distinct(Species)) %>%
        pull(speciesdiversity)
    })
  )

mean(Quercus_2017pairs_summary$speciesdiversity)

library(ggplot2)

ggplot(Quercus_2017pairs_summary, aes(x = speciesdiversity)) +
  geom_histogram(binwidth = 1, fill = "skyblue", color = "white") +
  labs(
    title = "Histogram of Beetle Species Richness per Trap Pair",
    x = "Species Richness",
    y = "Frequency"
  ) +
  theme_minimal()
...
Quercus_2017pairs_summary_xylo <- QuercusPairs2017 %>%
  mutate(
    n_xylobiont = map2_int(lower, upper, ~ {
      df_joined3 %>%
        filter(Falle %in% c(.x, .y), xylobiont == "j") %>%
        summarise(n = n_distinct(Species)) %>%
        pull(n)
    })
  )

mean(Quercus_2017pairs_summary_xylo$n_xylobiont)

  #redlist

Quercus_2017pairs_summary_rl <- QuercusPairs2017 %>%
  mutate(
    n_redlist = map2_int(lower, upper, ~ {
      df_joined3 %>%
        filter(Falle %in% c(.x, .y), Rote.Liste == "n") %>%
        summarise(n = n_distinct(Species)) %>%
        pull(n)
    })
  )

Quercus_2017_rl_fr <- tibble(
  rl_fraction = Quercus_2017pairs_summary$speciesdiversity - Quercus_2017pairs_summary_rl$n_redlist
)

mean(Quercus_2017_rl_fr$rl_fraction)

  #relictspecies

Quercus_2017pairs_summary_nonrelicts <- QuercusPairs2017 %>%
  mutate(
    n_relicts = map2_int(lower, upper, ~ {
      df_joined3 %>%
        filter(Falle %in% c(.x, .y), Urwaldrelikt == "n") %>%
        summarise(n = n_distinct(Species)) %>%
        pull(n)
    })
  )

Quercus_2017_relicts_fr <- tibble(
  rl_fraction = Quercus_2017pairs_summary$speciesdiversity - Quercus_2017pairs_summary_nonrelicts$n_relicts
)

mean(Quercus_2017_relicts_fr$rl_fraction)

#redlist_levels <- c("1", "2", "3", "n")

#Quercus_red_list2017 <- QuercusPairs2017 %>%
  mutate(
    RL_counts = map2(lower, upper, ~ {
      df_joined3 %>%
        filter(Rote.Liste %in% redlist_levels) %>%      
        distinct(Species, Rote.Liste) %>%
        count(Rote.Liste, name = "n_species") %>%
        complete(Rote.Liste = redlist_levels, fill = list(n_species = 0)) %>%
        arrange(Rote.Liste)
    })
  ) %>%
  unnest(RL_counts) %>%        
...
#Quercus_2017pairs_summary_redlist <- QuercusPairs2017 %>%
  mutate(
    n_redlist = map2(lower, upper, ~ {
      df_joined3 %>%
        filter(Falle %in% c(.x, .y),
               Rote.Liste %in% c("1", "2", "3")) %>%
        distinct(Species, Rote.Liste) %>% 
        count(Rote.Liste, name = "n_species")
      tidyr::complete(Rote.Liste = redlist_levels, fill = list(n_species = 0))
    })
  ) %>%
  tidyr::unnest(n_redlist) %>%
  tidyr::pivot_wider(
    names_from = Rote.Liste,
    values_from = n_species,
    values_fill = 0
...
lower_Tilia_traps_2017 <- df_joined3 %>%
  filter(TreeSp == "Tilia", Stratum == "Lower")%>%
  pull(Falle) %>%
  unique()

upper_Tilia_traps_2017 <- df_joined3 %>%
  filter(TreeSp == "Tilia", Stratum == "Upper")%>%
  pull(Falle) %>%
  unique()

set.seed(123)   
n_pairs <- 100
TiliaPairs2017 <- tibble(
  lower = sample(lower_Tilia_traps_2017, n_pairs, replace = TRUE),
  upper = sample(upper_Tilia_traps_2017, n_pairs, replace = TRUE)
)

  #speciesdiversity

Tilia_2017pairs_summary <- TiliaPairs2017 %>%
  mutate(
    speciesdiversity = map2_int(lower, upper, ~ {
      df_joined3 %>%
        filter(Falle %in% c(.x, .y)) %>%
...
mean(Tilia_2017pairs_summary$speciesdiversity)

  #xylobionts

Tilia_2017pairs_summary_xylo <- TiliaPairs2017 %>%
  mutate(
    n_xylobiont = map2_int(lower, upper, ~ {
      df_joined3 %>%
        filter(Falle %in% c(.x, .y), xylobiont == "j") %>%
...
mean(Tilia_2017pairs_summary_xylo$n_xylobiont)

  #redlist

Tilia_2017pairs_summary_rl <- TiliaPairs2017 %>%
  mutate(
    n_redlist = map2_int(lower, upper, ~ {
      df_joined3 %>%
        filter(Falle %in% c(.x, .y), Rote.Liste == "n") %>%
...
Tilia_2017_rl_fr <- tibble(
  rl_fraction = Tilia_2017pairs_summary$speciesdiversity - Tilia_2017pairs_summary_rl$n_redlist
)

mean(Tilia_2017_rl_fr$rl_fraction)

  #relictspecies

Tilia_2017pairs_summary_nonrelicts <- TiliaPairs2017 %>%
  mutate(
    n_relicts = map2_int(lower, upper, ~ {
      df_joined3 %>%
        filter(Falle %in% c(.x, .y), Urwaldrelikt == "n") %>%
...
Tilia_2017_relicts_fr <- tibble(
  rl_fraction = Tilia_2017pairs_summary$speciesdiversity - Tilia_2017pairs_summary_nonrelicts$n_relicts
)

mean(Tilia_2017_relicts_fr$rl_fraction)

#Fraxinus

lower_Fraxinus_traps_2017 <- df_joined3 %>%
  filter(TreeSp == "Fraxinus", Stratum == "Lower")%>%
  pull(Falle) %>%
  unique()

upper_Fraxinus_traps_2017 <- df_joined3 %>%
  filter(TreeSp == "Fraxinus", Stratum == "Upper")%>%
  pull(Falle) %>%
  unique()

set.seed(123)   
n_pairs <- 100
FraxinusPairs2017 <- tibble(
  lower = sample(lower_Fraxinus_traps_2017, n_pairs, replace = TRUE),
  upper = sample(upper_Fraxinus_traps_2017, n_pairs, replace = TRUE)
)

  #speciesdiversity

Fraxinus_2017pairs_summary <- FraxinusPairs2017 %>%
  mutate(
    speciesdiversity = map2_int(lower, upper, ~ {
      df_joined3 %>%
        filter(Falle %in% c(.x, .y)) %>%
...
mean(Fraxinus_2017pairs_summary$speciesdiversity)

  #xylobionts

Fraxinus_2017pairs_summary_xylo <- FraxinusPairs2017 %>%
  mutate(
    n_xylobiont = map2_int(lower, upper, ~ {
      df_joined3 %>%
        filter(Falle %in% c(.x, .y), xylobiont == "j") %>%
...
mean(Fraxinus_2017pairs_summary_xylo$n_xylobiont)

  #redlist

Fraxinus_2017pairs_summary_rl <- FraxinusPairs2017 %>%
  mutate(
    n_redlist = map2_int(lower, upper, ~ {
      df_joined3 %>%
        filter(Falle %in% c(.x, .y), Rote.Liste == "n") %>%
...
Fraxinus_2017_rl_fr <- tibble(
  rl_fraction = Fraxinus_2017pairs_summary$speciesdiversity - Fraxinus_2017pairs_summary_rl$n_redlist
)

mean(Fraxinus_2017_rl_fr$rl_fraction)

  #relictspecies

Fraxinus_2017pairs_summary_nonrelicts <- FraxinusPairs2017 %>%
  mutate(
    n_relicts = map2_int(lower, upper, ~ {
      df_joined3 %>%
        filter(Falle %in% c(.x, .y), Urwaldrelikt == "n") %>%
...
Fraxinus_2017_relicts_fr <- tibble(
  rl_fraction = Fraxinus_2017pairs_summary$speciesdiversity - Fraxinus_2017pairs_summary_nonrelicts$n_relicts
)

mean(Fraxinus_2017_relicts_fr$rl_fraction)

#2018bootstrapping

#Quercus

lower_Quercus_traps_2018 <- df_joined4 %>%
  filter(TreeSp == "Quercus", Stratum == "Lower")%>%
  pull(Falle) %>%
  unique()

upper_Quercus_traps_2018 <- df_joined4 %>%
  filter(TreeSp == "Quercus", Stratum == "Upper")%>%
  pull(Falle) %>%
  unique()
...
set.seed(123)   
n_pairs <- 100
QuercusPairs2018 <- tibble(
  lower = sample(lower_Quercus_traps_2018, n_pairs, replace = TRUE),
  upper = sample(upper_Quercus_traps_2018, n_pairs, replace = TRUE)
)

  #speciesdiversity

Quercus_2018pairs_summary <- QuercusPairs2018 %>%
  mutate(
    speciesdiversity = map2_int(lower, upper, ~ {
      df_joined4 %>%
        filter(Falle %in% c(.x, .y)) %>%
        summarise(speciesdiversity = n_distinct(Art.x)) %>%
        pull(speciesdiversity)
    })
  )

mean(Quercus_2018pairs_summary$speciesdiversity)

  #xylobionts

Quercus_2018pairs_summary_xylo <- QuercusPairs2018 %>%
  mutate(
    n_xylobiont = map2_int(lower, upper, ~ {
      df_joined4 %>%
        filter(Falle %in% c(.x, .y), xylobiont == "j") %>%
        summarise(n = n_distinct(Art.x)) %>%
        pull(n)
    })
  )

mean(Quercus_2018pairs_summary_xylo$n_xylobiont)

  #redlist

Quercus_2018pairs_summary_rl <- QuercusPairs2018 %>%
  mutate(
    n_redlist = map2_int(lower, upper, ~ {
      df_joined4 %>%
        filter(Falle %in% c(.x, .y), Rote.Liste == "n") %>%
...
Quercus_2018_rl_fr <- tibble(
  rl_fraction = Quercus_2018pairs_summary$speciesdiversity - Quercus_2018pairs_summary_rl$n_redlist
)

mean(Quercus_2018_rl_fr$rl_fraction)

  #relictspecies

Quercus_2018pairs_summary_nonrelicts <- QuercusPairs2018 %>%
  mutate(
    n_relicts = map2_int(lower, upper, ~ {
      df_joined4 %>%
        filter(Falle %in% c(.x, .y), Urwaldrelikt == "n") %>%
...
Quercus_2018_relicts_fr <- tibble(
  rl_fraction = Quercus_2018pairs_summary$speciesdiversity - Quercus_2018pairs_summary_nonrelicts$n_relicts
)

mean(Quercus_2018_relicts_fr$rl_fraction)

#Tilia

lower_Tilia_traps_2018 <- df_joined4 %>%
  filter(TreeSp == "Tilia", Stratum == "Lower")%>%
  pull(Falle) %>%
  unique()

upper_Tilia_traps_2018 <- df_joined4 %>%
  filter(TreeSp == "Tilia", Stratum == "Upper")%>%
  pull(Falle) %>%
  unique()
...
set.seed(123)   
n_pairs <- 100
TiliaPairs2018 <- tibble(
  lower = sample(lower_Tilia_traps_2018, n_pairs, replace = TRUE),
  upper = sample(upper_Tilia_traps_2018, n_pairs, replace = TRUE)
)

  #speciesdiversity

Tilia_2018pairs_summary <- TiliaPairs2018 %>%
  mutate(
    speciesdiversity = map2_int(lower, upper, ~ {
      df_joined4 %>%
        filter(Falle %in% c(.x, .y)) %>%
...
mean(Tilia_2018pairs_summary$speciesdiversity)

  #xylobionts

Tilia_2018pairs_summary_xylo <- TiliaPairs2018 %>%
  mutate(
    n_xylobiont = map2_int(lower, upper, ~ {
      df_joined4 %>%
        filter(Falle %in% c(.x, .y), xylobiont == "j") %>%
...
mean(Tilia_2018pairs_summary_xylo$n_xylobiont)

  #redlist

Tilia_2018pairs_summary_rl <- TiliaPairs2018 %>%
  mutate(
    n_redlist = map2_int(lower, upper, ~ {
      df_joined4 %>%
        filter(Falle %in% c(.x, .y), Rote.Liste == "n") %>%
...
Tilia_2018_rl_fr <- tibble(
  rl_fraction = Tilia_2018pairs_summary$speciesdiversity - Tilia_2018pairs_summary_rl$n_redlist
)

mean(Tilia_2018_rl_fr$rl_fraction)

  #relictspecies

Tilia_2018pairs_summary_nonrelicts <- TiliaPairs2018 %>%
  mutate(
    n_relicts = map2_int(lower, upper, ~ {
      df_joined4 %>%
        filter(Falle %in% c(.x, .y), Urwaldrelikt == "n") %>%
...
Tilia_2018_relicts_fr <- tibble(
  rl_fraction = Tilia_2018pairs_summary$speciesdiversity - Tilia_2018pairs_summary_nonrelicts$n_relicts
)

mean(Tilia_2018_relicts_fr$rl_fraction)

#Fraxinus

lower_Fraxinus_traps_2018 <- df_joined4 %>%
  filter(TreeSp == "Fraxinus", Stratum == "Lower")%>%
  pull(Falle) %>%
  unique()

upper_Fraxinus_traps_2018 <- df_joined4 %>%
  filter(TreeSp == "Fraxinus", Stratum == "Upper")%>%
  pull(Falle) %>%
  unique()
...
set.seed(123)   
n_pairs <- 100
FraxinusPairs2018 <- tibble(
  lower = sample(lower_Fraxinus_traps_2018, n_pairs, replace = TRUE),
  upper = sample(upper_Fraxinus_traps_2018, n_pairs, replace = TRUE)
)

  #speciesdiversity

Fraxinus_2018pairs_summary <- FraxinusPairs2018 %>%
  mutate(
    speciesdiversity = map2_int(lower, upper, ~ {
      df_joined4 %>%
        filter(Falle %in% c(.x, .y)) %>%
...
mean(Fraxinus_2018pairs_summary$speciesdiversity)

  #xylobionts

Fraxinus_2018pairs_summary_xylo <- FraxinusPairs2018 %>%
  mutate(
    n_xylobiont = map2_int(lower, upper, ~ {
      df_joined4 %>%
        filter(Falle %in% c(.x, .y), xylobiont == "j") %>%
...
mean(Fraxinus_2018pairs_summary_xylo$n_xylobiont)

  #redlist

Fraxinus_2018pairs_summary_rl <- FraxinusPairs2018 %>%
  mutate(
    n_redlist = map2_int(lower, upper, ~ {
      df_joined4 %>%
        filter(Falle %in% c(.x, .y), Rote.Liste == "n") %>%
...
Fraxinus_2018_rl_fr <- tibble(
  rl_fraction = Fraxinus_2018pairs_summary$speciesdiversity - Fraxinus_2018pairs_summary_rl$n_redlist
)

mean(Fraxinus_2018_rl_fr$rl_fraction)

  #relictspecies

Fraxinus_2018pairs_summary_nonrelicts <- FraxinusPairs2018 %>%
  mutate(
    n_relicts = map2_int(lower, upper, ~ {
      df_joined4 %>%
        filter(Falle %in% c(.x, .y), Urwaldrelikt == "n") %>%
...
Fraxinus_2018_relicts_fr <- tibble(
  rl_fraction = Fraxinus_2018pairs_summary$speciesdiversity - Fraxinus_2018pairs_summary_nonrelicts$n_relicts
)

mean(Fraxinus_2018_relicts_fr$rl_fraction)
...
write.xlsx(xylobionts2017, file = "xylobionts2017.xlsx", sheetName = "Sheet1", overwrite = TRUE)
